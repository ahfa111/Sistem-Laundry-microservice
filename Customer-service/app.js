...
app.post("/customer-reviews", async (req, res) => {
  ...
  if (!customer_id || !rating)
    return res
      .status(400)
      .json({ message: "customer_id and rating are required" });
  ...
  try {
    const [result] = await pool.query(
      ...
      [customerIdNum, ratingNum, review_text || null],
    );
    res
      .status(201)
      .json({
...
app.put("/customer-reviews/:id", async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      ...
      [ratingNum, review_text || null, req.params.id],
    );
    if (result.affectedRows === 0)
      return res.status(404).json({ message: "Review not found" });
    res.json({ id: parseInt(req.params.id), rating: ratingNum, review_text });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
